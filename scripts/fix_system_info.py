...
def fix_vulnerability_category(file_path: str, category: str = "应用系统漏洞", dry_run: bool = False) -> Tuple[bool, str]:
    """修复VL页面的漏洞大类字段
    ...
    try:
        ...
        # 2. 检查是否有vulnerability_category字段
        fm_end = -1
        has_category = False
        for i in range(1, len(lines)):
            if lines[i].strip() == '---':
            ...
            if lines[i].strip().startswith('vulnerability_category:'):
                has_category = True
                # 检查值是否正确
                if category in lines[i]:
                    ...
                    # 需要修复
                    if not dry_run:
                        lines[i] = f"vulnerability_category: \"{category}\""
                        with open(file_path, 'w', encoding='utf-8') as f:
                            f.write('\n'.join(lines))
        ...
        # 3. 如果没有vulnerability_category字段，添加它
        if not has_category and fm_end != -1:
            # 在frontmatter结束前添加
            new_lines = lines[:fm_end] + [f"vulnerability_category: \"{category}\""] + lines[fm_end:]
...
def batch_fix(directory: str, dry_run: bool = False, fix_category: bool = False, category: str = "应用系统漏洞"):
    ...
    for filename in sorted(vl_files):
        ...
        # 修复漏洞大类字段
        if fix_category:
            modified2, msg2 = fix_vulnerability_category(filepath, category, dry_run)
            print(f"    {msg2}")
...
def main():
    ...
    if args.file:
        ...
        if args.fix_category:
            modified2, msg2 = fix_vulnerability_category(args.file, args.category, args.dry_run)
            print(f"  {msg2}")
